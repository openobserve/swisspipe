...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    use swisspipe::async_execution::mpsc_worker_pool::MpscWorkerPoolConfig;
    ...
    let mut input_sync_manager = swisspipe::workflow::input_sync_manager::InputSyncManager::new(db.clone());
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            match swisspipe::database::sessions::Entity::delete_many()
                .filter(swisspipe::database::sessions::Column::ExpiresAt.lt(now))
            ...
            match swisspipe::database::csrf_tokens::Entity::delete_many()
                .filter(swisspipe::database::csrf_tokens::Column::ExpiresAt.lt(now))
                .exec(&*session_cleanup_db)
                .await
            {
                Ok(result) => {
            ...
            match swisspipe::database::csrf_tokens::Entity::delete_many()
                .filter(swisspipe::database::csrf_tokens::Column::Used.eq(true))
                .filter(swisspipe::database::csrf_tokens::Column::CreatedAt.lt(twenty_four_hours_ago))
                .exec(&*session_cleanup_db)
                .await
            {
                Ok(result) => {
    ...
    let encryption = match swisspipe::variables::EncryptionService::from_env() {
        Ok(enc) => Arc::new(enc),
        Err(e) => {
    ...
    let variable_service = Arc::new(swisspipe::variables::VariableService::new(
        db.clone(),
        (*encryption).clone(),
    ));

    // Initialize template engine
    let template_engine = Arc::new(swisspipe::variables::TemplateEngine::new());

    // Inject variable service and template engine into workflow engine
    engine.set_variable_service(variable_service.clone())?;
    ...
    if let Err(e) = worker_pool.shutdown().await {
        tracing::error!("Error shutting down worker pool: {}", e);
    }
